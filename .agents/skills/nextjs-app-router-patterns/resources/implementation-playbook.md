...
export async function addToCart(rawProductId: string) {
  const { productId } = cartActionSchema.parse({ productId: rawProductId })
...
const checkoutSchema = z.object({
  ...
  payment: z.enum(['credit_card', 'paypal']),
})
...
export async function checkout(formData: FormData) {
  ...
  const { address, payment } = validatedFields.data
