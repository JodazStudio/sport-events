...
#### Actions
1. Design component hierarchy using a **`features/`** directory and **Barrel Exports**
2. Create base components and feature-specific components with an `index.ts`
3. Implement layout components and streaming boundaries
4. Set up state management (Client only for leaves)
5. Create custom hooks within features
...
#### Actions
1. Implement Server Components as the default for data fetching
2. Use **Streaming with Suspense** for slow data sources
3. Set up React Query/SWR if client-side caching is needed
4. Create API clients with **Zod** validation
...
2. **Mandatory validation with Zod**
3. Create form components (Client leaves)
4. Handle submissions with **Server Actions** for end-to-end type safety
5. Implement error handling and validation feedback
...
| Architecture| Feature-driven (`features/`) |
| SEO | `generateMetadata`, `next/image` |
| Performance| Streaming (`Suspense`), RSC |
